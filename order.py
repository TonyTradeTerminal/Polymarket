...
def process_side(client, 
    ...
    if current_buy_orders.get(token):
        logger.info(f"[{question}] {token.upper()} 买单存在，订单ID: {current_buy_orders[token][0].get('id')}")
        ...
        buy_order_times = int(current_buy_orders[token][0].get("created_at"))

        ######################################################################################
        # 1.1 检查买单状态，如果 超时 并且 best bid 发生变化那么就cancel重新下单，如果没超时 或 best bid 无变化 就查看买单是否全部成交 （2）
        ######################################################################################
        if int(time.time()) - buy_order_times > 30 and float(best_bid_price) != float(best_bid[token]):  # 超过一定时间 并且 最优买单价格发生了变化
            ...
            if buy_matched_size is not None and float(buy_matched_size) == float(0):
                ...
                buy_order_times = 0 # 4 删除旧的买单时间记录
                ...
                if buy_orders_resp.get(token):
                    ...
                    buy_order_times = int(current_buy_orders[token][0].get('created_at'))  # 3 更新 buy_order_times
        ...
        if not any(order.get("id") == buy_order_id for order in client.get_orders(OpenOrderParams(market=market_id))): # 若在更新的订单中找不到该买单，认为订单已全部成交
                ...
                buy_order_id = '' #2 清空 buy_order_id
                best_bid[token] = [] # 3 清空 best bid
                buy_order_times = 0  # 4 删除成交的买单时间记录
        else:
        ...
        if not current_buy_orders.get(token) and not current_sell_orders.get(token): # 如果买单没有记录 且 卖单还没有记录
            ...
            sell_orders_rep = place_sell_orders(client, 
                                                token_ids, 
                                                market_id, 
            ...
            if sell_orders_rep and sell_orders_rep.get(token):
                            
                #####更新######
                current_sell_orders[token] = sell_orders_rep[token] # 1 更新sell orders

                logger.info(f"[{question}] {token.upper()} 卖单已下单，订单ID: {current_sell_orders[token][0].get('id')}")
            else:
                logger.warning(f"[{question}] {token.upper()} 卖单下单失败。")

        else:
            logger.warning(f"[{question}] {token.upper()} 下卖单条件不成立 ")
    ...
    if current_sell_orders.get(token):        
        ...
        if not any(order.get("id") == current_sell_orders[token][0].get("id") for order in client.get_orders(OpenOrderParams(market=market_id))):
            ...
            if buy_orders_resp.get(token):
                ...
                buy_order_times = int(current_buy_orders[token][0].get('created_at'))  # 2 更新 buy_order_times
